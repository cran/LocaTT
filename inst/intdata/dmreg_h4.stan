...
data{
  ...
  int<lower=1> L4; // Number of hierarchical effects (#4).
  int<lower=1> h4[N]; // Vector of hierarchical identifiers (#4).
...
parameters{
  ...
  matrix[D-1,L4] H4_raw; // Raw hierarchical effects matrix (#4).
  vector<lower=0>[D-1] sigma2_h4; // Hierarchical variance vector (#4).
...
transformed parameters{
  ...
  matrix[D,L4] H4; // Hierarchical effects matrix (#4).
  vector<lower=0>[D-1] sigma4; // Hierarchical standard deviation vector (#4).
...
model{
  ...
  for(d in 1:(D-1)){
    for(l in 1:L4){
      target += normal_lpdf(H4_raw[d,l]|0,sigma4[d]);
    }
  }
  
  // Hierarchical variances (#4).
  for(d in 1:(D-1)){
    target += inv_gamma_lpdf(sigma2_h4[d]|sigma2_alpha,sigma2_beta);
  }
  ...
  for(i in 1:N){
    ...
    for(d in 1:D){
      ...
      eta[d]=B[d,1:K]*transpose(X[i,1:K])+H1[d,h1[i]]+H2[d,h2[i]]+H3[d,h3[i]]+H4[d,h4[i]];
    }
